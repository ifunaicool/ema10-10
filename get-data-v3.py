...
def filter_a_stock_codes(codes_list):
    """筛选有效的A股代码"""
    valid_codes = []
    ...
    for code in codes_list:
        ...
        # 标准化为6位
        if len(code_str) > 6:
            code_str = code_str[-6:]  # 取最后6位
        elif len(code_str) < 6:
            code_str = code_str.zfill(6)

        # 检查是否为有效的A股代码格式
        if code_str.startswith(('00', '30', '60')) and len(code_str) == 6:
...
if __name__ == "__main__":
    ...
    # 获取单只股票
    df = fetcher.get_stock_historical_data('000001', days=30)
    if not df.empty:
    ...
    results = fetcher.get_multiple_stocks_data_batch(codes, days=30)
    print(f"批量获取成功 {len(results)} 只股票数据")
